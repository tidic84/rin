...
module.exports = {
    ...
    async execute(client, interaction, queue) {
        ...
        default:
            let status = "none";
            if (queue.repeatMode === 3) { status = "autoplay"; } 
            else if (queue.repeatMode === 2) { status = "queue"; } 
            else if (queue.repeatMode === 1) { status = "track"; } 
            else if (queue.repeatMode === 0) { status = "off"; }
        ...
        embeds: [new EmbedBuilder().setDescription(description).setColor(client.colors.blue)],
        });
    },
};